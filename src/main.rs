...
impl Game {
    ...
    fn near_seed_station(&self) -> bool {
        self.farmer.position.distance(SEED_STATION_POS) < 2.5
    }
}
...
fn draw_potato_plant(center: Vec3, growth: f32) {
    ...
    if growth > 0.85 {
        ...
        draw_sphere(center + vec3(-0.15, 0.12, 0.12), 0.12, None, potato);
        draw_sphere(center + vec3(0.15, 0.1, -0.1), 0.11, None, potato);
    }
}
...
fn draw_seed_station() {
    ...
    draw_cube(
        SEED_STATION_POS + vec3(0.0, 0.3, 0.0),
        vec3(1.5, 0.6, 1.5),
        None,
        Color::from_rgba(150, 100, 45, 255),
    );

    // Machine top

    draw_cylinder(
        SEED_STATION_POS + vec3(0.0, 0.9, 0.0),
        0.45,
        0.45,
        0.7,
        None,
...
fn draw_hud(game: &Game) {
    draw_text("WASD / Arrows - Move", 16.0, 28.0, 22.0, BLACK);
    draw_text("SPACE - Plow", 16.0, 54.0, 22.0, BLACK);
    draw_text("E - Plant / Harvest / Use station", 16.0, 80.0, 22.0, BLACK);
    draw_text(
    ...
    let status = match game.field[gx][gz] {
        ...
        CellState::Planted { growth } if growth >= 1.0 => "Ready - press E to harvest".to_string(),
        CellState::Planted { growth } => format!("Growing... {}%", (growth * 100.0) as u32),
    };

    draw_text(&status, 16.0, 180.0, 22.0, DARKGREEN);
}
